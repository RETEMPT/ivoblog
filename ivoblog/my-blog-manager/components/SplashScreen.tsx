...
export default function SplashScreen() {
  ...
  useEffect(() => {
    setIsMounted(true);
    if (siteConfig.effectsConfig?.enableSplashScreen === false) {
    ...
    if (!hasSeenSplash) {
      ...
      const timer = setTimeout(() => {
        exitSplash();
      }, siteConfig.effectsConfig?.performanceMode === 'performance' ? 500 : 900);
          ...
          <div className="relative z-10 flex flex-col items-center">
            ...
            <div className="w-40 h-[1.5px] bg-slate-200 dark:bg-slate-800 relative">
              <motion.div
                initial={{ width: "0%" }}
                animate={{ width: "100%" }}
                transition={{ duration: reduceMotion ? 0.01 : 1.8, ease: "easeInOut" }}
