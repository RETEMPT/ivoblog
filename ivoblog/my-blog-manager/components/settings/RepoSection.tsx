...
export default function RepoSection() {
  ...
  useEffect(() => {
    const fetchConfig = async () => {
      try {
        const res = await fetchBackend("/api/deploy/config");
        if (res.ok) {
        ...
        showToast("Python 后端未连接，请先运行根目录 start-manager.bat。", "warning");
      }
    };
    fetchConfig();
  }, [showToast]);
  ...
  const testPathConnection = async () => {
    ...
    try {
      const res = await fetchBackend("/api/sync/check", {
        method: 'POST',
  ...
  const testGitConnection = async () => {
    ...
    try {
      const res = await fetchBackend("/api/deploy/check", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ blogPath: deployData.blogPath })
      });
  ...
  const handleGetSSH = async (type: 'static' | 'source') => {
    try {
      ...
      const res = await fetchBackend(`/api/deploy/ssh/key?type=${type}`);
      const data = await res.json();
      if (data.success) {
  ...
  const executeInitEnv = async () => {
    ...
    try {
      const res = await fetchBackend("/api/deploy/init", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const executeDeploy = async () => {
    ...
    try {
      const res = await fetchBackend("/api/deploy/publish", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ blogPath: deployData.blogPath })
      });
  ...
  const executeUploadSource = async () => {
    ...
    try {
      const res = await fetchBackend("/api/deploy/source", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ blogPath: deployData.blogPath })
      });
