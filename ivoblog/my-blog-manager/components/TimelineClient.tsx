...
import { Plus, Search, AlertTriangle, Sparkles, LayoutGrid, ListTree, Edit3, ArrowUp } from 'lucide-react';
import { useToast } from './ToastProvider';
import Link from 'next/link';
import { fetchBackendJson } from '../lib/backendClient';
import type { Post } from '../types/post';

const DEFAULT_COVER = "/uploads/images/_20251225232527_213_2-0d4cd615c3.jpg";

function postDate(value?: string) {
  return (value || "").split(" ")[0] || "未注明日期";
}
...
export default function TimelineClient({ posts: initialPosts, tags }: { posts: Post[], tags: { name: string, count: number }[] }) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const data = await fetchBackendJson<{ success: boolean; message?: string }>('/api/drafts/delete', {
        method: 'POST',
      ...
      if (data?.success) {
        ...
        showToast("❌ 销毁失败: " + (data?.message || "后端未连接"), "error");
      }
    } catch {
      showToast("无法连接到 Python 引擎，请通过 start-manager.bat 启动管理端", "error");
    } finally {
                    ...
                    <div className="bg-white/60 dark:bg-slate-800/70 backdrop-blur-xl border border-white/50 dark:border-slate-700/50 rounded-3xl overflow-hidden shadow-lg flex flex-col h-full group relative">
                      ...
                      <Link href={`/posts/${post.slug}`} className="block flex-1 flex flex-col cursor-pointer">
                        <div className="relative h-40 overflow-hidden">
                          <img src={post.cover || DEFAULT_COVER} alt={post.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
