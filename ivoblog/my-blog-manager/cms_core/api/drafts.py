...
from cms_core.security import atomic_write_json, atomic_write_text, safe_join, validate_safe_id
...
async def save_draft(request: Request):
    ...
    if not draft_id or draft_id == 'new':
        ...
        draft_id = validate_safe_id(draft_id, "draft id")
    ...
    file_path = safe_join(drafts_dir, f"{draft_id}.json")
    try:
        atomic_write_json(file_path, draft_data)
...
async def get_draft(request: Request):
    ...
    raw_id = validate_safe_id(payload.get("id", "").replace(".md", ""), "draft id")
    ...
    # 1. 优先从草稿箱读取 JSON
    file_path = safe_join(drafts_dir, f"{raw_id}.json")
    ...
    if raw_id == "about" or doc_type == "about":
        target_md = safe_join(base_dir, "app", "about", "about.md")
    else:
        folder = "posts" if doc_type == "post" else "chatters"
        target_md = safe_join(base_dir, folder, f"{raw_id}.md")
...
async def delete_draft(request: Request):
    ...
    raw_id = validate_safe_id(payload.get("id", "").replace(".md", "").replace(".json", ""), "draft id")
    # 🌟 修复：用 PROJECT_ROOT 替换 os.getcwd()
    base_dir = PROJECT_ROOT
    drafts_dir = get_manager_drafts_dir()

    possible_paths = [
        safe_join(drafts_dir, f"{raw_id}.json"),
        safe_join(base_dir, "posts", f"{raw_id}.md"),
        safe_join(base_dir, "chatters", f"{raw_id}.md")
    ]
    ...
    for op in operations:
        if op.get("type") == "publish_article":
            ...
            if not final_id or final_id == 'new':
                ...
                final_id = validate_safe_id(final_id, "document id")
            ...
            if doc_type == "about":
                save_path = safe_join(base_dir, "app", "about", "about.md")
            else:
                folder = "posts" if doc_type == "post" else "chatters"
                save_path = safe_join(base_dir, folder, f"{final_id}.md")

            atomic_write_text(save_path, final_text)

            draft_path = safe_join(drafts_dir, f"{validate_safe_id(doc_id, 'draft id')}.json") if doc_id else ""
