...
export default function DijiangModel({ posts = [], chatters = [], moments = [] }: any) {
  ...
  useEffect(() => {
    ...
    const fetchGitalkComments = async () => {
      ...
      try {
        ...
        if (!issueRes.ok) throw new Error("GitHub issues unavailable");
        const issues = await issueRes.json();

        if (issues && issues.length > 0) {
          const commentsRes = await fetch(issues[0].comments_url, { signal: controller.signal });
          if (!commentsRes.ok) throw new Error("GitHub comments unavailable");
  ...
  const currentMonthRecords = useMemo(() => {
    const formatted = [...posts, ...chatters, ...moments].map(r => ({
      ...
      image: r.cover || r.image || '/uploads/images/_20251225232527_213_2-0d4cd615c3.jpg',
      content: r.content
