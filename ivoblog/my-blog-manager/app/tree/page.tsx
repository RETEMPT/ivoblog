...
function getLocalItems(dirPath: string, typeName: string) {
  let items: any[] = [];
  try {
    ...
    console.error(`读取 ${typeName} 失败:`, error);
  }
  return items;
}

export default function CreativeWorkshopPage() {
  const posts = getLocalItems(postsDirectory, 'post');
  const chatters = getLocalItems(chattersDirectory, 'chatter');
  const moments = getLocalItems(momentsDirectory, 'moment');
