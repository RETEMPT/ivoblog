...
export default function OperatorRecreation() {
  ...
  useEffect(() => {
    ...
    const triggerRandomTalk = () => {
      ...
      setShowBubble(true);
      scheduleTimeout(() => setShowBubble(false), 4500); // 停留时间
    };

    const initialTimeout = scheduleTimeout(triggerRandomTalk, 3000);
    const talkInterval = setInterval(() => {
      if (document.hidden) return;
      scheduleTimeout(triggerRandomTalk, Math.random() * 3000);
    }, 12000);
