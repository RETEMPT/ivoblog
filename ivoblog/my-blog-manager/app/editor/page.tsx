...
function EditorContent() {
  ...
  useEffect(() => {
    const fetchTags = async () => {
      ...
      try {
        const data = await fetchBackendJson<{ success: boolean; postTags?: string[]; chatterTags?: string[] }>('/api/drafts/all_tags');
        if (data?.success) {
  ...
  useEffect(() => {
    if (currentDocId !== 'new') {
      const loadDraft = async () => {
        try {
          const data = await fetchBackendJson<{ success: boolean; draft?: any; message?: string }>('/api/drafts/get', {
            method: 'POST',
          ...
          if (data?.success && data.draft) {
            ...
            setCover(normalizeLocalAssetPath(data.draft.cover || '', ''));
        ...
        } catch { showToast("❌ 读取失败，请确认 Python 后端已启动", "error"); }
      };
      loadDraft();
    }
  }, [currentDocId, docType, showToast]);

  useEffect(() => {
    if (!isLoadingTags) {
      setHasUnsavedChanges(true);
    }
  }, [title, tags, cover, summary, mood, isLoadingTags]);
  ...
  const handleSave = async (isPublish: boolean, shouldExitAfterSave: boolean = false) => {
    ...
    const payload = {
      ...
      type: docType, title, tags, cover: normalizeLocalAssetPath(cover, ''), mood, description: summary,
    ...
    try {
      ...
      if (data?.success) {
        ...
        showToast(data?.message || "❌ 保存失败，请确认 Python 后端已启动", "error");
      }
    } catch { showToast("❌ 保存失败，请确认 Python 后端已启动", "error"); }
  ...
  return (
    <div className="h-screen w-full overflow-hidden relative">
      ...
      <PageTransition>
        <main className="mx-auto w-[96%] max-w-[1750px] flex flex-row gap-6 relative" style={{ marginTop: '144px', height: 'calc(100vh - 144px - 32px)', marginBottom: '32px' }}>
          ...
          <aside className="w-[360px] shrink-0 bg-white/30 dark:bg-slate-800/40 backdrop-blur-[60px] rounded-[50px] shadow-2xl border border-white/30 dark:border-white/10 flex flex-col overflow-hidden">
            <MetaMatrix
              type={docType as any} tags={tags} setTags={setTags} cover={cover} setCover={(val) => setCover(sanitizeLocalAssetInput(val))} summary={summary} setSummary={setSummary} mood={mood} setMood={setMood}
      ...
      <FloatingImageTool isOpen={isImgToolOpen} onClose={() => setIsImgToolOpen(false)} onInsert={(url) => {
        if (imgToolTarget === 'editor') { editorRef.current?.insertImage(url); if (!cover) setCover(normalizeLocalAssetPath(url, '')); }
        else { setCover(normalizeLocalAssetPath(url, '')); setIsImgToolOpen(false); }
        setHasUnsavedChanges(true);
      }} />
    </div>
  );
