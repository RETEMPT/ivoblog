...
import { MapPin, MessageSquare, Clock, Sparkles, Search, ArrowDownAZ, ArrowUpZA, ChevronLeft, ChevronRight, Ghost, Plus, Image as ImageIcon, X, Send, Zap, Trash2, AlertTriangle } from 'lucide-react';
...
export default function MomentList({ moments, authorName, avatarUrl }: any) {
  const [localMoments, setLocalMoments] = useState<any[]>(() => (Array.isArray(moments) ? moments : []));
  ...
  useEffect(() => {
    setLocalMoments(Array.isArray(moments) ? moments : []);
  }, [moments]);

  const processedMoments = useMemo(() => {
    let baseMoments = [...localMoments];
  ...
  const handleFileUpload = async (files: FileList | File[]) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        else showToast(`第 ${i + 1} 张上传失败: ${data?.message || "后端未连接"}`, "error");
      }
      if (newUrls.length > 0) {
  ...
  const handleDirectPublish = async () => {
    ...
    try {
      ...
      const data = await fetchBackendJson<{ success: boolean; message?: string }>('/api/moments/save', {
        method: 'POST',
      ...
      if (data?.success) {
        showToast("发布成功，已更新当前列表", "success");
        setLocalMoments(prev => [payload, ...prev]);
        setIsPublishOpen(false);
        setNewMoment({ content: '', location: '', images: [] });
      } else {
        showToast(`⚠️ 后端拒绝了请求：${data?.message || "后端未连接"}`, "error");
      }
    } catch (error: any) {
      showToast(`🚨 请求彻底断裂：${error.message || "请确认 Python 后端已启动"}`, "error");
    } finally {
  ...
  const executeDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const data = await fetchBackendJson<{ success: boolean; message?: string }>('/api/moments/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data?.success) {
        ...
        setLocalMoments(prev => prev.filter(moment => moment.id !== deleteConfirmId));
      } else {
        showToast(`删除失败: ${data?.message || "后端未连接"}`, "error");
      }
    } catch (error: any) {
      showToast(`连接失败: ${error.message || "请确认 Python 后端已启动"}`, "error");
    } finally {
      setIsDeleting(false);
