...
function getDirActivities(dirPath: string, idPrefix: string, typeLabel: '文章' | '杂谈' | '说说', linkPrefix: string) {
  try {
    ...
    console.error(`读取 ${dirPath} 活动数据失败:`, e);
    return [];
  }
}

export default async function AboutPage() {
  const fullPath = path.join(/*turbopackIgnore: true*/ process.cwd(), 'app', 'about', 'about.md');
  let contentHtml = "博主很懒，还没有写自我介绍哦...";
  let coverImage = siteConfig.defaultPostCover;
  ...
  const posts = getDirActivities(postsDirectory, 'posts', '文章', 'posts');
  const chatters = getDirActivities(chattersDirectory, 'chatters', '杂谈', 'chatter');
  const moments = getDirActivities(momentsDirectory, 'moments', '说说', 'moments');
